...
import {NavigationProps, StackNavigationProps} from '../navigation/types';
...
export const Product: React.FC<NavigationProps> = ({navigation, route}) => {
  ...
  useEffect(() => {
    (async () => {
      let res = await getProduct(route?.params?.id);
      if ((res as Product)?.attributeValues) {
        res = res as Product;
        const attributeToObject: any = res.attributeValues.reduce(
          (attribute, index) => {
  ...
  if (isError && errorPage) {
    let title = '';
    let description = '';
    errorPage.attributeValues?.map(attribute => {
      if (attribute?.error_title) {
        title = attribute.error_title;
      }
      if (attribute?.error_description) {
        description = attribute.error_description[0].value;
      }
    });
