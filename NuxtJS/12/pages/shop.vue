...
import { useGetProductPages, useGetProductsConfig, useSearchProducts } from '../composables/states'

const products = await useGetProductPages()
const config = await useGetProductsConfig()
...
async function getProductsByPage(page) {
    const response = await fetch(`https://nuxt-projects.oneentry.ru/api/content/products/page/url/shop?limit=${limit.value}&offset=${(page - 1) * limit.value}&langCode=en_US&sortOrder=DESC&sortKey=id`, {
        method: 'get',
        headers: {
...
async function searchProducts() {
    if (searchStr.value.length) {
        const findProducts = await useSearchProducts(searchStr.value)
        if (findProducts.length) {
            const result = []
            await Promise.all(
                findProducts.map(async (product) => {
                    await useGetProductById(product.id).then((res) => {
                        result.push(res)
                    })
                })
            )
            actualProducts.value = [result]
        } else {
...
async function filterProduct() {
    ...
    const filteredProducts = await useFilterProducts(data)
